...
First thing to do is to set the org-roam directory.
#+begin_src emacs-lisp
...
***** Findings of usage
It looks like =emacs-jest= executes =npm test --= with a few different parameters which means that you would need to ensure in your package.json you have =test= script.

There is the custom variable =jest-executable= which we can use to override the command. Perhaps we adjust this to execute the jest binary in =node_modules= of the current project. If we do this what happens to the other parameters the module generates?

#+begin_src emacs-lisp
;; (setq jest-executable "/usr/local/bin/node ./node_modules/.bin/jest ")
#+end_src
The jest plugin for vs code executes =/usr/local/bin/node ./node_modules/.bin/jest <filename> -c <path to jest config> -t <test name> -- runInBand=. I wonder if we can make =emacs-jest= generate something like that
